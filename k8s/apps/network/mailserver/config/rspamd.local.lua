...
local ABUSIX_API_KEY = "{{ .abusixApiKey }}"
...
local check_shorturls_cb = function(task)
    ...
    if not check_shorturls_dns then
        return false
    end
...
local check_diskurls_cb = function(task)
    -- Disable checks if no DNS namespace is set-up
    if not check_diskurls_dns then
        return false
    end
...
local check_web_submission_ips_cb = function(task)
    -- Disable checks if no DNS namespace is set-up
    if not check_web_submission_dns then
        return false
    end
